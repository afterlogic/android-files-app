package com.afterlogic.aurora.drive.data.modules.project7.modules.files.repository;
...
public class FilesRepository7Impl extends BaseRepository implements FilesRepository {
    ...
    @Inject public FilesRepository7Impl(@RepositoryCache ObservableCache cache,
    ...
    public Single<Collection<AuroraFile>> getFiles(AuroraFile folder) {
        return withNetMapper(
                ...
                files -> MapperUtil.collection(mFileNetToBlMapper)
                        .map(files.getFiles())
        );
    }
    ...
    public Single<Boolean> delete(List<AuroraFile> files) {
        return Single.defer(() -> {
            ...
            List<AuroraFileP7> mapped = MapperUtil.collection(mFileBlToNetMapper).map(files);
